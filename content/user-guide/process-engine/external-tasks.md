...
Further information on the functionality of error event definitions on external tasks can be found in the [expression language user guide]("/user-guide/process-engine/expression-language.md#external-task-error-handling"). The specific use in external tasks for RPA orchestration scenarios is described in the [Camunda Platform RPA Bridge](./user-guide/camunda-bpm-rpa-bridge.md#error-handling).
